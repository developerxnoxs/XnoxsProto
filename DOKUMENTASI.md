...
> **DC Migration Otomatis:** Library secara otomatis mendeteksi apakah file berada di DC yang berbeda dari session aktif. Jika berbeda, koneksi sementara ke DC file dibuat, auth di-export/import, lalu file diunduh dari sana. Tidak perlu konfigurasi tambahan.

> **FILE_REFERENCE_EXPIRED:** Untuk dokumen (video, audio, file), library secara otomatis me-refresh file_reference yang kadaluarsa. Ketika Telegram mengembalikan error ini, library re-fetch pesan asli dari Telegram, ambil file_reference baru, dan lanjutkan download — tanpa perlu tindakan dari user. Behaviour ini mengikuti implementasi Telethon. (Hanya untuk dokumen; foto tidak perlu refresh.)

### 24.1 Download dari Pesan History (Cara Termudah)

> **Catatan penting:** `getHistory()` mengembalikan flat array langsung — **bukan** `['messages' => [...]]`.
> Iterasi langsung atas hasil `getHistory()`, bukan `$result['messages']`.

```php
// Ambil history — media sudah ter-parse lengkap (id, access_hash, file_reference, dc_id)
...
**Dengan progress callback (menampilkan persen nyata untuk dokumen):**
```php
...
> **Progress callback untuk dokumen:** `$total` berisi ukuran file lengkap (bytes) dan `$pct` adalah persentase 0–100.
> Untuk foto, `$total = 0` dan `$pct = 0` karena ukuran foto tidak diketahui sebelum download selesai.

### 24.2 Struktur Array `media` dalam Pesan

Setiap pesan dari `getHistory()` yang memiliki media akan punya field `media` berisi:
...
$msg['media'] = [
    ...
    'id'             => 7485920374...,    // Media ID (dibutuhkan untuk download)
    'access_hash'    => -5821038473...,   // Access hash
    'file_reference' => "\x01\x02...",   // File reference bytes
    'dc_id'          => 4,               // DC tempat file disimpan
    'size'           => 1048576,         // Ukuran file dalam bytes (dokumen)
    'thumb_size'     => 'y',             // Ukuran foto terbaik yang tersedia (foto)
];
```

**Cara cek dan download berdasarkan tipe:**
```php
$media = $msg['media'];

switch ($media['type']) {
    case 'photo':
        echo "Foto, DC={$media['dc_id']}, thumb={$media['thumb_size']}\n";
    ...
    case 'audio':
    case 'voice':
        echo "Audio: {$media['mime']}\n";
        $ext = $client->getMediaExtension($media);
        $client->downloadMedia($msg, "/tmp/audio_{$msg['id']}.$ext");
        break;
...
- Membutuhkan `_message_obj` dalam array pesan (tersedia otomatis dari `getHistory()`)
