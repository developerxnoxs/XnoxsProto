...
class FileSession extends AbstractSession
{
    ...
    // ── Key derivation ───────────────────────────────────────────────────
    //
    // Keys are machine-local: derived from hostname + real file path.
    // This makes session files non-portable (like Telethon's SQLite sessions).
    ...
    {
        // realpath() returns false when the file does not yet exist (first save).
        // To get a stable key we resolve the *directory* (which always exists after
        // mkdir) and append the basename — giving a consistent absolute path both
        // before and after the file is created.
        $dir      = realpath(dirname($this->filename));
        if ($dir === false) {
            // Directory itself doesn't exist yet — fall back to the raw path.
            $dir = dirname($this->filename);
        }
        $stablePath = $dir . DIRECTORY_SEPARATOR . basename($this->filename);

        $base   = gethostname() . ':XnoxsProto:' . $stablePath;
