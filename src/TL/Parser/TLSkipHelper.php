...
class TLSkipHelper
{
    ...
    /**
     * Read a Document and return info: ['type', 'mime', 'filename'].
     * Consumes all document bytes from the reader.
     */
    private static function readDocumentInfo(BinaryReader $r): array
    {
        $c = $r->readInt();
        if ($c === 0x36f8c871) { // documentEmpty
            $r->readLong();
        ...
        $r->readBytes();  // file_reference
        $r->readInt();    // date
        $mime = $r->readString(); // mime_type
        $r->readLong();   // size
        if ($flags & (1 << 0)) self::skipVector($r, fn($x) => self::skipPhotoSize($x));
        if ($flags & (1 << 1)) self::skipVector($r, fn($x) => self::skipVideoSize($x));
        $r->readInt();    // dc_id

        // Read attributes vector to determine sub-type and filename
        $docType  = 'document';
        $filename = '';
        $r->readInt(); // vector ctor 0x1cb5c415
        $num = $r->readInt();
        for ($i = 0; $i < $num; $i++) {
            ...
            switch ($ac) {
                ...
                case 0x43c57c48: // video attribute
                    $aFlags = $r->readInt();
                    $r->readDouble(); $r->readInt(); $r->readInt();
                ...
                case 0x9852f9c6: // audio attribute
                    $aFlags = $r->readInt();
                    $r->readInt();
        ...
        return ['type' => $docType, 'mime' => $mime, 'filename' => $filename];
    }
